...
function StatCard({ s, mobile, onStatClick }: { s: StatItem; mobile?: boolean; onStatClick?: (action: string) => void }) {
  const content = (
  ...
  if (s.action && onStatClick) {
    return <button onClick={() => onStatClick(s.action!)} className="text-left w-full">{content}</button>;
  }
  return <>{content}</>;
}
...
export default function StatsPanel({ stats, mobile, onStatClick }: Props) {
  if (mobile) {
    return (
      <div className="flex gap-1.5 overflow-x-auto pb-1 mb-2">
        {stats.map(s => <StatCard key={s.label} s={s} mobile onStatClick={onStatClick} />)}
      </div>
    );
  }
  ...
  return (
    <div className="fixed right-6 top-1/2 -translate-y-1/2 z-20 flex flex-col gap-2">
      {stats.map(s => <StatCard key={s.label} s={s} onStatClick={onStatClick} />)}
    </div>
  );
}
