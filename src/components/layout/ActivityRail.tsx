...
function ActivityCard({ a, onSelect }: { a: ActivitySummary; onSelect: (a: ActivitySummary) => void }) {
  const sport = SPORT_MAP[a.sportType] ?? { name: '未知', color: '#666' };
  return (
    <button onClick={() => onSelect(a)}
      className="flex items-center gap-2.5 min-w-[225px] max-md:min-w-[180px] max-md:flex-shrink-0 rounded-xl border border-[rgba(255,255,255,0.06)] bg-[rgba(10,10,10,0.92)] px-3.5 py-2.5 text-left backdrop-blur-xl transition hover:bg-[rgba(15,15,15,0.96)] hover:border-white/12">
...
export default function ActivityRail({ activities, onSelect, mobile }: Props) {
  ...
  if (mobile) {
    return (
      <div className="flex gap-2 overflow-x-auto pb-1">
        {recent.map(a => <ActivityCard key={a.id} a={a} onSelect={onSelect} />)}
      </div>
    );
  }
